# ============================================================================
# COMPANY RESEARCH AUTOMATION - PERPLEXITY v8.7 - COMPANY NAME ONLY QUERIES
# ============================================================================
# UPDATES APPLIED:
# 1. ✓ Query changed: Company name ONLY (no additional details)
# 2. ✓ Example: "microsoft" NOT "microsoft company website address..."
# 3. ✓ Cleaner search queries for better results
# 4. ✓ Enhanced thread instructions for maximum accuracy
# 5. ✓ Fresh URL navigation per company (no follow-up chains)
#
# SETUP INSTRUCTIONS:
# 1. Update your thread instructions (see enhanced_thread_instructions.md)
# 2. Ensure creden.json and .env are in same folder
# 3. Run: python company_research_v8.7_OPTIMIZED.py
#
# ============================================================================
...
# ============================================================================
# OPTIMIZED SEARCH - COMPANY NAME ONLY
# ============================================================================

async def navigate_and_search_optimized(page, company_name, search_num, client):
    """Navigate fresh and search with COMPANY NAME ONLY - OPTIMIZED"""
    try:
        ...
        # Type company name ONLY (no additional details)
        ...
        # Type COMPANY NAME ONLY - OPTIMIZED
        query = company_name +("company")  # JUST THE COMPANY NAME
...
async def extract_all_company_data_optimized_v87(page, client, company_name):
    """Extract data from optimized search - COMPANY NAME ONLY"""
    try:
        print(f"\n{'='*80}")
        ...
        # OPTIMIZED SEARCH: Company name only
        screenshot, text = await navigate_and_search_optimized(page, company_name, 1, client)
...
async def process_company_optimized_v87(page, client, company_name, sheet, row_num):
    """Process company using optimized search"""
    try:
        results = await extract_all_company_data_optimized_v87(page, client, company_name)
...
async def main():
    ...
    logger.info(TITLE)
    logger.info("COMPANY RESEARCH v8.7 - OPTIMIZED (COMPANY NAME ONLY)")
    logger.info(TITLE)
    
    print(TITLE)
    print("COMPANY RESEARCH v8.7 - OPTIMIZED")
    print("✓ Query: Company name ONLY (no extra details)")
    print("✓ Fresh URL navigation per company")
    print("✓ Single optimized search per company")
    print("✓ All data extracted from one result")
    print("✓ Enhanced thread instructions for accuracy")
    print(TITLE)
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            for i, company_data in enumerate(companies, 1):
                print(f"\n[{i}/{len(companies)}]")
                result = await process_company_optimized_v87(
